...
export class AclCtrl {
  dashboard: any;
  items: DashboardAcl[];
  permissionOptions = [{ value: 1, text: 'بیننده' }, { value: 2, text: 'ویراش' }, { value: 4, text: 'مدیر' }];
  aclTypes = [
  ...
  prepareViewModel(item: DashboardAcl): DashboardAcl {
    ...
    if (item.userId > 0) {
      ...
      item.nameHtml = this.$sce.trustAsHtml(`همه افراد در نقش <span class="query-keyword">${item.role}</span>`);
